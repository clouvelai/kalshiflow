...
class KeyDriverAnalysis:
    ...
    """
    # Primary driver (default to empty for error recovery contexts)
...
class Evidence:
    ...
    # Metadata: Additional context from evidence sources (e.g., Truth Social engagement metrics)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "evidence": {
                ...
                "sources_checked": self.evidence.sources_checked,
                "reliability": self.evidence.reliability.value,
                "reliability_reasoning": self.evidence.reliability_reasoning,
...
class MarketAssessment:
    ...
    - base_rate_used: The base rate LLM started from
    - adjustment_up: Evidence pushing probability up
    - adjustment_up_reasoning: What evidence justified increase
    - adjustment_down: Evidence pushing probability down
    - adjustment_down_reasoning: What evidence justified decrease
    """
    market_ticker: str
    market_title: str
    ...
    # === Base Rate Anchoring Fields (v3 additions - all with defaults for backward compatibility) ===
    base_rate_used: float = 0.5         # The base rate LLM started from
    adjustment_up: float = 0.0          # Evidence pushing probability up (0.0-0.5)
    adjustment_up_reasoning: str = ""   # What evidence justified increasing from base rate
    adjustment_down: float = 0.0        # Evidence pushing probability down (0.0-0.5)
    adjustment_down_reasoning: str = "" # What evidence justified decreasing from base rate

    # === Extreme Probability Monitoring (v4.1 addition) ===
    extreme_probability_flag: Optional[str] = None  # Set if probability was clamped from extreme value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adjustment_up": self.adjustment_up,
            "adjustment_up_reasoning": self.adjustment_up_reasoning,
            "adjustment_down": self.adjustment_down,
            "adjustment_down_reasoning": self.adjustment_down_reasoning,
            # v4.1 extreme probability monitoring
            "extreme_probability_flag": self.extreme_probability_flag,
        }
