"""
PositionListener - WebSocket listener for market position updates.

This service connects to the Kalshi user WebSocket and subscribes to the 
"market_positions" channel to receive real-time position updates. Position 
changes are forwarded to the OrderManager for state synchronization.

Architecture:
    KalshiWS (market_positions channel) -> PositionListener -> OrderManager.update_position_from_websocket()
...
Note: All monetary values (position_cost, realized_pnl, fees_paid) are in 
centi-cents (1/10,000th of a dollar). Divide by 10,000 to convert to dollars.
"""
...
logger = logging.getLogger("kalshiflow_rl.trading.position_listener")
...
class PositionListener:
    """
    WebSocket listener for Kalshi market position updates.
    
    Connects to Kalshi WebSocket, subscribes to the "market_positions" channel, and
    forwards position update messages to the OrderManager for state synchronization.
    ...
    def __init__(
        ...
        Args:
            order_manager: OrderManager instance with update_position_from_websocket() method
    ...
    async def _connect_and_listen(self) -> None:
        """
        Connect to WebSocket and listen for position update messages.
        """
        if not self._auth:
    ...
    async def _handle_message(self, raw_message: str) -> None:
        ...
        Parses message and forwards position update events to OrderManager.
    ...
    async def _handle_position_message(self, message: Dict[str, Any]) -> None:
        """
        Handle a position update message from Kalshi.
        
        Extracts position data and forwards to OrderManager.
        ...
        position_cost = position_data.get("position_cost", 0)  # in centi-cents
        realized_pnl = position_data.get("realized_pnl", 0)  # in centi-cents
        fees_paid = position_data.get("fees_paid", 0)  # in centi-cents
        volume = position_data.get("volume", 0)
        
        logger.info(
            f"📊 Position update: market={market_ticker}, "
            f"position={position}, cost={position_cost}, "
            f"realized_pnl={realized_pnl}, fees={fees_paid}, volume={volume}"
        )
        
        # Convert centi-cents to cents for monetary values
        # Kalshi returns monetary values in centi-cents (1/10,000th of a dollar)
        # We store everything in cents (divide by 100) for consistency
        position_data_converted = {
            "market_ticker": market_ticker,
            "position": position,
            "position_cost": position_cost / 100.0,  # Convert centi-cents to cents
            "realized_pnl": realized_pnl / 100.0,  # Convert centi-cents to cents
            "fees_paid": fees_paid / 100.0,  # Convert centi-cents to cents
            "volume": volume,
            "raw_data": position_data  # Keep raw data for reference
        }
        
        # Forward to OrderManager via update_position_from_websocket()
        try:
            await self.order_manager.update_position_from_websocket(position_data_converted)
            self._positions_processed += 1
            logger.debug(f"Position update processed: {market_ticker}")
        except Exception as e:
            logger.error(f"Failed to process position update {market_ticker}: {e}")
    ...
    def get_last_sync_time(self) -> Optional[float]:
        """
        Get last position update received time.
        
        Returns:
            Timestamp of last position update received, or None if no updates yet
        """
        metrics = self.get_metrics()
        return metrics.get("last_position_time")
